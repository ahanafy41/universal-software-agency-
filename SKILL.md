...
## 2. Cross-Platform Runtime & Tool Mapping

The agency seamlessly adapts to the hosting AI agent platform:
- **Google Antigravity 2.0 & CLI**: Native file editing (`view_file`, `replace_file_content`), bash execution (`run_command`), workspace subagents (`.agents/agents/` via `invoke_subagent`), and search tools.
- **Gemini Spark & Web Environments**: Shell execution (`vm_shell:execute_bash`), file tools (`default_api:view_file`), subagents (`invoke_subagent`), and search (`google:search`).

---

## 3. Core Engineering Tracks
...
## 4. Dual Scoping Protocol & Project Manifests
...
## 5. Mandatory Hybrid Reference Ingestion Protocol
...
## 6. Human-Grade Engineering & Senior Craftsmanship
...
## 7. Mandatory Pre-Fix Diagnostic & Triage Gate
...
## 8. Strict Code Preservation & Zero-Corruption Invariant
...
## 9. Automated Testing & Verification Seal
...
## 10. 3-Tier Security Boundaries

- **Always Allowed**: Read/write in project directory, run static analysis and unit tests, search Google/web for official docs.
- **Ask First**: Adding external dependencies, running migration scripts, modifying schemas.
- **Never Touch**: Production credentials, `.env*` secret files, files outside the project workspace.

---

## 11. Progressive Disclosure Reference Index

When specific operational details or templates are required, consult the 3 consolidated playbooks, the unified schema, and automation scripts:
